...
module "ecs" {
  ...
  image                     = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.region}.amazonaws.com/english-somali-dictionary-app-ecr-repo${data.aws_ecr_image.app_image.image_tag}"
  region                    = var.region
